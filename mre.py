...
def test_from_import_1():
    ...
    for channel in range(1,105):
        ...
        timer = time.time()
        k_max = 1500
        slopes =  np.ma.empty([len(block.segments), k_max-1])
        stderrs = np.ma.empty([len(block.segments), k_max-1])
        ...
        print("t seperate: ", time.time()-timer)
        ...
        sols=[]
        for i in res.steps:
            solc.append(func(i, mr_resc['popt'][0], mr_resc['popt'][1]))
            sols.append(func(i, mr_ress['popt'][0], mr_ress['popt'][1]))
        ax2.plot(res.steps, solc, color='red',
                     label=func(0, mr_resc['popt'][0], mr_resc['popt'][1],
                     string=True))
        ax2.plot(res.steps, sols, color='blue',
                     label=func(0, mr_ress['popt'][0], mr_ress['popt'][1],
                     string=True))
        ax2.legend(loc='upper right')

        plt.tight_layout()
        plt.savefig('/home/pspitzner/owncloud/mpi/temp/plot_{}.svg'.format(channel))
        plt.close()
        # plt.show()
